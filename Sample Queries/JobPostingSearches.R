...
https://www.library.hbs.edu/services/research-data-faculty-doctoral
June 2025
Updated: August 2026
"
...
"
The postings_cosmos table contains aggregated job posting data.
...
res <- dbSendQuery(wrds, glue(
  "SELECT job_id, company, country, state, metro_area, role_k1500_v2, 
  salary, post_date
...
"
The role variables (role_k1500_v2 etc.) are Revelio's classificaitons of a 
job role. The original role title posted to a job site in stored in the 
postings_cosmos_raw table.

This query pulls basic informaiton on job postings from the postings_cosmos 
table. It also pulls the original job title and description from the 
postings_cosmos_raw table.
...
res <- dbSendQuery(wrds, glue(
     "SELECT p.job_id, p.company, p.country, p.state, p.metro_area, 
     p.role_k1500_V2, p.salary,
            r.title_raw, r.description 
      FROM revelio.postings_cosmos as p
      LEFT JOIN revelio.postings_cosmos_raw as r
